...
def decoding(encoding, in_channels, out_size, n_classes):
  ...
  features = []
  classifier = []
  #in_channels = 3 # change here
  #out_size = 28 #256  # change here
  prev = -1
  pos = 0
  o_sizes = []
  ...
  classifier += [nn.Linear(n_neurons, n_classes)]
        ...
        for c in range(len(connections)):
          if connections[c] == 1:
            ...
            if skip_size > req_size:
              ...
              x2 = pool(outputs[c])
              #print(f'Layer: {i}, Pooled skip connection shape: {x2.shape}')
            if skip_size == req_size:
              x2 = outputs[c]
              #print(f'Layer: {i}, Skip connection shape (sizes match): {x2.shape}')
            if skip_size < req_size:
                if (req_size - skip_size) % 2 == 0:
                  ...
                  if pad > 0:
                    padding = nn.ZeroPad2d(pad)
                    x2 = padding(x2)
             #print(f'Layer: {i}, Adjusted skip connection shape: {x2.shape}')
            x = torch.cat((x, x2), axis = 1)
            #print(f'Layer: {i}, Shape after concatenation: {x.shape}')
